...
import {
  ...
  togglePluginSkill,
} from '../../../settings/plugins/data/plugin-connectors.ts'
...
test('/ 只展示启用技能、已连接 MCP 和管理入口', () => {
  ...
  assert.deepEqual(
    groups.find((group) => group.id === 'mcp')?.items.map((item) => item.label),
    ['网页搜索', '管理 MCP'],
  )
})
...
test('设置变化会返回 Skill 与 MCP 的失效原因', () => {
  ...
  assert.equal(
    getComposerContextUnavailableReason(disabledSkill, skills, mcpServers),
    '技能已禁用',
  )
  assert.equal(
    getComposerContextUnavailableReason(offlineMcp, skills, mcpServers),
    'MCP 未连接',
  )
})
