...
import { Clock, ListTimeline, Magnifier, Wrench } from '@gravity-ui/icons'
...
export function AgentTraceView() {
  ...
  const records = useMemo(() => {
    ...
    return MOCK_AGENT_TRACE.records.filter((record) =>
      `${record.label} ${record.preview} ${record.source}`
        .toLowerCase()
        .includes(query),
    )
  }, [deferredSearch])
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background px-4 sm:pr-6 sm:pl-10">
      <div className="shrink-0">
        <div className="flex flex-wrap items-center gap-2 border-b border-separator py-1">
          ...
          <div className="relative min-w-40 max-w-56 flex-1 sm:flex-none">
            <Magnifier className="pointer-events-none absolute top-1/2 left-2 z-10 size-3.5 -translate-y-1/2 text-muted" />
            <TextField
              aria-label="搜索轨迹记录"
              fullWidth
              value={search}
      ...
      <div className="flex min-h-0 flex-1 lg:flex-row">
        ...
        {selectedRecord ? (
          <div className="flex min-h-0 min-w-0 flex-1 flex-col border-separator lg:w-[clamp(320px,38%,440px)] lg:max-w-[calc(100%-280px)] lg:flex-none lg:border-l">
            <TraceDetailPanel
